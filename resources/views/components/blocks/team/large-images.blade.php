            ...
            @foreach ($data['team_members'] as $member)
                <li>
                    <img class="aspect-[3/2] w-full rounded-2xl object-cover"
                        ...
                        alt="">
                    <h3 class="mt-6 text-lg/8 font-semibold tracking-tight text-gray-900">{{ $member['name'] }}</h3>
                    <p class="text-base/7 text-gray-600">{{ $member['title'] }}</p>
                    <ul role="list" class="mt-6 flex gap-x-6">
                        @foreach ($member['social_links'] as $link)
                            <li>
                                <a href="{{ $link['url'] }}" class="text-gray-400 hover:text-gray-500" target="_blank">
